...
netsuiteRoutes.get('/connections', async (c) => {
  const user = c.get('user');
  const conns = await db.select().from(connections).where(eq(connections.tenantId, user.tenantId));
  return c.json(conns.map(redactConnection));
});

netsuiteRoutes.get('/connections/:id', async (c) => {
  const user = c.get('user');
  const [conn] = await db.select().from(connections).where(eq(connections.id, c.req.param('id'))).limit(1);
...
// Trigger full sync: subsidiaries → COA → trial balance
