...
importRoutes.post('/trial-balance', zValidator('json', importSchema), async (c) => {
  ...
  await createMissingMappings(user.tenantId, Array.from(importedAccountIds));
...
async function createMissingMappings(tenantId: string, accountIds: string[]) {
  if (accountIds.length === 0) return;

  const existingMappings = await db.select().from(taxMappings)
    .where(and(
      eq(taxMappings.tenantId, tenantId),
      inArray(taxMappings.accountId, accountIds),
    ));
  const mappedAccountIds = new Set(existingMappings.map(mapping => mapping.accountId));
  const unmappedAccountIds = accountIds.filter(accountId => !mappedAccountIds.has(accountId));
  if (unmappedAccountIds.length === 0) return;

  const importedAccounts = await db.select().from(accounts)
    .where(and(
      eq(accounts.tenantId, tenantId),
      inArray(accounts.id, unmappedAccountIds),
    ));

  const results = fallbackClassify(importedAccounts.map(account => ({
    id: account.id,
  ...
  for (const result of results) {
    await db.insert(taxMappings).values({
      ...
      taxAccountType: result.taxAccountType,
      bookTreatment: result.bookTreatment,
...
importRoutes.get('/trial-balance/export', async (c) => {
  ...
  const entityRows = entityIds.length > 0
    ? await db.select().from(entities).where(and(eq(entities.tenantId, user.tenantId), inArray(entities.id, entityIds)))
    : [];
  const entityMap = new Map(entityRows.map(e => [e.id, e]));

  const accountRows = accountIds.length > 0
    ? await db.select().from(accounts).where(and(eq(accounts.tenantId, user.tenantId), inArray(accounts.id, accountIds)))
    : [];
