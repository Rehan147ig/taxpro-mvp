...
| 3A.2 | Apportionment weights (per Tax Foundation TaxEDU, "State Primary Apportionment Factors for Tax Year 2026", captured 2026-08-03): single sales factor (0/0/1) default; equal three-factor (1/3/1/3/1/3) for AK, HI, KS, ND, NM, OK; double-weighted sales (0.25/0.25/0.5) for FL, VA | A reviewer verifies the formula per state (KS enacted SSF in 2024 — confirm effective date; OK has an SSF election) | A state uses a hybrid or industry-specific formula not captured here — weights must be corrected per state |
...
| 3A.9 | Ruleset freshness is enforced by the verifier (`npm run verify:us-rates`): `STATE_RULESET` must match the latest dated external snapshots in `external-snapshots.ts` — rates vs Tax Foundation 2026 (captured 2026-04-02) and apportionment weights vs Tax Foundation TaxEDU 2026 (captured 2026-08-03); 51/51 rows exact. A state changing rates/structure/weights fails CI until the snapshot is re-fetched AND the ruleset updated | The snapshot is re-fetched on every legislative change | The snapshot is edited to match the ruleset instead of vice versa, or a stale snapshot passes unnoticed |
| 3A.10 | The agentic rule-refresh loop (`rule-update-agent` in apps/api + `proposals.ts` contract in this package) extracts rule changes from dated legal sources into machine-readable proposals; a proposal is never applied to the ruleset — a CPA approves it, then the ruleset AND snapshot are updated together so the verifier and engine tests gate the change | Human review approves every proposal; ruleset+snapshot move atomically | A proposal is applied without approval, or the snapshot is updated without the ruleset (verifier fails) |
