...
- **CI (GitHub Actions, `.github/workflows/`, green on master):** 4 workflows per push/PR —
  `ci.yml` (Security Scan: Gitleaks advisory + Trufflehog; Lint & Test on a fresh Postgres 16 + Redis 7: bootstrap roles → migrate → seed → 479 tests; Docker Build & Scan: API/Web + Trivy HIGH/CRITICAL), `codeql.yml` (CodeQL security + extended), `semgrep.yml` (p/security-audit + TS/JS, 0 findings), `deps.yml` (OSV dependency gate). Dependabot enabled for npm / Actions / Docker.
